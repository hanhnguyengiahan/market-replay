...
class AddOrderTest : public testing::Test {
  protected:
...
TEST_F(AddOrderTest, EmptyBookHasNoPriceLevels) {
    EXPECT_TRUE(orderBook.getPriceLevels("BUY").empty());
    EXPECT_TRUE(orderBook.getPriceLevels("SELL").empty());
}

TEST_F(AddOrderTest, BuyAndSellOrdersStoredSeparately) {
...
TEST_F(AddOrderTest, QuantityAccumulatedIfSamePrice) {
...
TEST_F(AddOrderTest, HighestBidsReturnFromPriceLevels) {
...
TEST_F(AddOrderTest, LowestSellsReturnFromPriceLevels) {
...
// Trade
TEST_F(AddOrderTest, NoTradeWhenPricesDoNotCross) {
...
TEST_F(AddOrderTest, FullyMatchedOrdersAreRemoved) {
...
TEST_F(AddOrderTest, IncomingOrderCanBePartiallyFilled) {
    auto sell = createOrder(1, 150.00, 100, "SELL");
